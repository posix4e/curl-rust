...
pub fn test_get_requests() {
    ...
    assert!(res1.get_body().unwrap() == "Hello".as_bytes());

    assert!(res2.get_code() == 200);
    assert!(res2.get_body().unwrap() == "World".as_bytes());
}
...
pub fn test_post_get_requests() {
    ...
    assert!(res1.get_code() == 200);
    assert!(res1.get_body().unwrap() == "World".as_bytes(), "actual={}",
            String::from_utf8_lossy(res1.get_body().unwrap()));

    assert!(res2.get_code() == 200);
    assert!(res2.get_body().unwrap() == "NEXT".as_bytes(), "actual={}",
            String::from_utf8_lossy(res2.get_body().unwrap()));
}
